...
def train_main_network(x_train, y_train):
    encaps_function = EncapsulatedFunctionsDistributedPytorch(num_workers=1)
    torch_model = MLP()
    torch_model.apply(init_weights)
    criterion = nn.MSELoss
    optimizer = optim.Adam
    optimizer_parameters = {"lr": 0.0002}
    encaps_function.build(torch_model, optimizer, criterion, optimizer_parameters, num_gpu=1, num_nodes=1)
