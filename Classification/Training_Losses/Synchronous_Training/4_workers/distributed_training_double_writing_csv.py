...
def train_main_network(x_train, y_train, x_test, y_test):
    ...
    trained_weights, train_loss, train_acc, validation_loss, val_acc = encaps_function.fit_synchronous_with_GPU(x_train, y_train, num_batches, 50, shuffle_blocks=False, shuffle_block_data=False, return_loss=True, x_test=x_test, y_test=y_test) 
    ...
    df = pd.DataFrame([train_loss])#[tensor.tolist() for tensor in train_loss])
    ...
    df = pd.DataFrame([train_acc])#tensor.tolist() for tensor in train_acc])
