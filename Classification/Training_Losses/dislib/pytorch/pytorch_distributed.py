...
class PytorchDistributed(object):
    ...
    def _train_cnn_batch_GPU(self, model_parameters, x_train,
        ...
        output_list = []
        translator = RandomTranslate(translate=(0.1, 0.1))
        rotator = RandomRotation(degrees=15)
        ...
        return self.model, np.array(losses_epoch).mean(), accuracy_score(true_labels.detach().numpy(), output_list.detach().numpy())
