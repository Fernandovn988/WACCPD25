...
class EncapsulatedFunctionsDistributedPytorch(object):
    ...
    def fit_synchronous_with_GPU(self, x_train, y_train,
                                 ...
                                 make_validation=True, return_loss=False, x_test=None, y_test=None):
        """
        ...
        for i in range(num_epochs):
            ...
            if x_test is not None:
                ...
                outputs = outputs.detach().cpu().numpy()
                validation_acc.append(accuracy_score(y_test_local, outputs))
            torch_model = torch_model.train().to("cpu")
    ...
    def fit_asynchronous_with_GPU(self, x_train, y_train,
                                  ...
                                  y_test=None,
                                  shuffle_blocks=True,
                                  shuffle_block_data=True, return_loss=False):
        """
            ...
            for j in range(self.num_workers):
                parameters_for_workers[j] = \
                    self.compss_object[j].aggregate_parameters_async(
                        self.model_parameters,
                        parameters_for_workers[j])
            val_loss, val_acc = evaluate_model_parameters(x_test_local, y_test_local, y_test_one_hot_encoded,
                    self.compss_object[0].model, parameters_for_workers[-1])
        ...
        training_loss = [np.array(epoch_loss).mean() for epoch_loss in training_loss]
        training_acc = [np.array(epoch_accuracy).mean() for epoch_accuracy in training_acc]
        if return_loss:
            return self.model_parameters, training_loss, training_acc, validation_loss, validation_acc
        else:
            return self.model_parameters
