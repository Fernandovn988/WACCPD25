...
class NFBlockBN(nn.Module):
    """Bloque residual de NFNet with BatchNorm"""
    def __init__(self, in_channels, out_channels, stride=1, alpha=0.2):
        super().__init__()
        self.alpha = alpha
        if stride != 1 or in_channels != out_channels:
...
class NFNet(nn.Module):
    ...
    def __init__(self, num_classes=10, alpha=0.2):
        ...
        self.neural_network_layers = nn.Sequential(
                ...
                NFBlockBN(64, 128, stride=1, alpha=alpha),
                NFBlock(128, 256, stride=1, alpha=alpha),
                NFBlock(256, 512, stride=2, alpha=alpha),
                NFBlock(512, 1024, stride=2, alpha=alpha),
