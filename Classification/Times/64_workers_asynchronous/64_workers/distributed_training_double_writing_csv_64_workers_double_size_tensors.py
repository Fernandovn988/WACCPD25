from NFNet import NFNet, init_weights, assign_weigths_to_model
import torch
import dislib as ds
import pandas as pd
import numpy as np
...
from dislib.data.tensor import Tensor, cat
import sys
...
def load_data(x_train, y_train, x_test, y_test):
    ...
    x_train_3 = torch.load(x_train).float()
    x_train = torch.cat((x_train_1, x_train_2, x_train_3))
    x_train = ds.data.tensor.from_pt_tensor(x_train, shape=(64, 1))
    y_train_1 = torch.load(y_train).int()
    y_train_2 = torch.load(y_train).int()
    y_train_3 = torch.load(y_train).int()
    y_train = torch.cat((y_train_1, y_train_2, y_train_3))
    gc.collect()
...
def train_main_network(x_train, y_train, x_test, y_test):
    encaps_function = EncapsulatedFunctionsDistributedPytorch(num_workers=64)
    ...
    encaps_function.build(torch_model, optimizer, criterion, optimizer_parameters, scheduler=scheduler, T_max=100, eta_min=0.0001, num_gpu=4, num_nodes=16) 
    start_time = time.time()
    trained_weights = encaps_function.fit_asynchronous_with_GPU(x_train, y_train, num_batches, 50) 
